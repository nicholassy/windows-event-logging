...
USAGE:
  .\FilteredEventsToPdf.ps1 -StartDate "2026-01-01 00:00:00" -EndDate "2026-01-31 23:59:59"
  .\FilteredEventsToPdf.ps1 -DaysBack 30 -OutPdf "C:\Reports\event_report.pdf" -IncludeMessage
...
Modified:
- Handle cases where there is no offender. (i.e. 4625)
#>
...
Write-Host "Log collection window:" -ForegroundColor Cyan
...
function Get-FieldValue {
    ...
    try {
        ...
        foreach ($name in $FieldNames) {
            $hit = $nodes | Where-Object { $_.Name -eq $name } | Select-Object -First 1
            if ($hit -and $hit.'#text' -and $hit.'#text'.Trim() -and $hit.'#text'.Trim() -ne "-") {
...
function New-FilteredEventObject {
    ...
    if ($id -eq 4625) {
        ...
        $acct = Get-FieldValue -Event $Event -FieldNames @(
            ...
            "SubjectUserName"      # fallback
        )

        $dom = Get-FieldValue -Event $Event -FieldNames @(
            "TargetDomainName",
            "AccountDomain",
            "SubjectDomainName"
        )

        # If still blank, try to parse from message text
        if (-not $acct -or $acct -eq "-") {
            try {
                $msg = $Event.Message
                if ($msg -match '(?m)^\s*Account Name:\s*(.+?)\s*$') {
                    $acct = $Matches[1].Trim()
                    if ($acct -eq "-") { $acct = $null }
                }
                if ($msg -match '(?m)^\s*Account Domain:\s*(.+?)\s*$') {
                    $dom = $Matches[1].Trim()
                    if ($dom -eq "-") { $dom = $null }
                }
            } catch {}
        ...
        if ($dom -and $acct) {
            ...
            # last resort: keep the event (don't drop it), mark unknown
            $offender = "UNKNOWN (4625)"
        }
    }

    # Normalize system accounts comparison properly
    $SystemAccountsUpper = $SystemAccounts | ForEach-Object { $_.ToUpper() }
    ...
    switch ($id) {
        ...
        4625 {
            ...
            $ip   = Pick @("IpAddress","WorkstationName")
            $status = Pick @("Status")
            $subStatus = Pick @("SubStatus")
            $reason = Pick @("FailureReason")
            $proc = Pick @("ProcessName")

            $base.Description = "Failed logon (Type $logonType) - Offender: $offender - From: $ip"
            $base.Details = "LogonType=$logonType; IP/WS=$ip; Status=$status; SubStatus=$subStatus; Reason=$reason; Process=$proc"
        }
...
Time window: $StartTime to $EndTime<br/>
Logs scanned: $((($LogsToQuery | ForEach-Object {$_.LogName}) -join ", "))<br/>
System excluded: $($SystemAccounts -join ", ")
</div>
